...
if ($this->getMethodeRequete() != "GET") {
    ...
    if ($pseudo == "" || $mdpSha1 == "" || $pseudoAsupprimer == "") {
        ...
        $code_reponse = 400;
    } else { // il faut être administrateur pour supprimer un utilisateur
        if ($dao->getNiveauConnexion($pseudo, $mdpSha1) != 2) {
            $msg = "Erreur : authentification incorrecte.";
            $code_reponse = 401;
        } else { // contrôle d'existence de pseudoAsupprimer
            $unUtilisateur = $dao->getUnUtilisateur($pseudoAsupprimer);
            if ($unUtilisateur == null) {
                $msg = "Erreur : pseudo utilisateur inexistant.";
                $code_reponse = 400;
            } else { // si cet utilisateur possède encore des traces, sa suppression est refusée
