...
$mdpSha1 = (empty($dao->request['a'])) ? "" : $dao->request['a'];
$pseudoAutorisant = (empty($dao->request['b'])) ? "" : $dao->request['b'];
$pseudoAutorise = (empty($dao->request['c'])) ? "" : $dao->request['c'];
$decision = (empty($dao->request['d'])) ? "" : $dao->request['d'];
...
if ($dao->getMethodeRequete() != "GET") {
    ...
    if ($mdpSha1 == "" || $pseudoAutorisant == "" || $pseudoAutorise == "" || ($decision != 0 && $decision != 1)) {
        ...
        $code_reponse = 400;
    } else { // test de l'authentification de l'utilisateur
            ...
            if ($dao->autoriseAConsulter($idAutorisant, $idAutorise)) {
                ...
                { // enregistrement de l'autorisation dans la bdd
                    ...
                    if (! $ok) {
                        ...
                        $code_reponse = 500;
                    } else { // envoi d'un mail d'acceptation à l'intéressé
